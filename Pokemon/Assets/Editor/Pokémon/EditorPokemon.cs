...
namespace Mfknudsen.Editor.Pokémon
{
    ...
    public class EditorPokemon : UnityEditor.Editor
    {
        ...
        private Color standard = new Color(0.2f, 0.2f, 0.2f);
        ...
        private void OnEnable()
        {
            ...
            pokemons = new List<Mfknudsen.Pokémon.Pokemon>();
            ...
            for (int i = 0; i < allPokemonAssets.Length; i++)
            {
                ...
                Mfknudsen.Pokémon.Pokemon pokemonAsset = AssetDatabase.LoadAssetAtPath<Mfknudsen.Pokémon.Pokemon>(assetPath);
        ...
        {
            standard = GUI.color;
            script = (Mfknudsen.Pokémon.Pokemon)target;
        ...
        private void ShowStats()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                //Stat
                script.SetStat((Stat)i, EditorGUILayout.IntField(script.GetStatRaw((Stat)i), GUILayout.Height(h), GUILayout.Width(w)));
                statTotal += script.GetStatRaw((Stat)i);
                //IV
                script.SetIV((Stat)i, EditorGUILayout.IntField(script.GetIV((Stat)i), GUILayout.Height(h), GUILayout.Width(w)));
                ivTotal += script.GetIV((Stat)i);
                //EV
                script.SetEV((Stat)i, EditorGUILayout.IntField(script.GetEV((Stat)i), GUILayout.Height(h), GUILayout.Width(w)));
        ...
        private void ShowType()
        {
            ...
            if (toDisplay[1] == null)
            {
                ...
                if (EditorGUILayout.Toggle(false))
                    toDisplay = new Type[] { toDisplay[0], types[Random.Range(1, toDisplay.Length - 1)] };
                EditorGUILayout.EndHorizontal();
            }
        ...
        private void ShowBreeding()
        {
            ...
            GUILayout.Label("Male: " + script.GetGenderRate() + "% | Female: " + (100 - script.GetGenderRate()) + "%", textStyle);
            EditorGUILayout.EndVertical();
            //
            //
            EditorGUILayout.BeginVertical(EditorStyles.helpBox, GUILayout.Height(90));
        ...
        private void ShowMoves()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            if (showLearnset)
                GUILayout.Label("Hide Learnset", GUILayout.Width(90));
            else
                GUILayout.Label("Show Learnset", GUILayout.Width(90));
            ...
            if (showLearnset)
            {
                ...
                List<PokemonMove> resultValue = new List<PokemonMove>();
                ...
                Mfknudsen.Pokémon.Pokemon[] pokemonKeys = script.GetBreedingLearnableMoveKeys();
                values = script.GetBreedingLearnableMoveValue();

                List<Mfknudsen.Pokémon.Pokemon> resultValuePoke = new List<Mfknudsen.Pokémon.Pokemon>();
                resultValue = new List<PokemonMove>();

                for (int i = 0; i < pokemonKeys.Length; i++)
                {
                    Mfknudsen.Pokémon.Pokemon pokemonKey = pokemonKeys[i];
                    PokemonMove value = values[i];
